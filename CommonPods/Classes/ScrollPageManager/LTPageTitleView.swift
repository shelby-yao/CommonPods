...
@objc public class LTPageTitleView: UIView {
    ...
    private lazy var glt_titleRGBlColor: (r : CGFloat, g : CGFloat, b : CGFloat) = getRGBWithColor(layout.ltTitleLayout.titleColor ?? NORMAL_BASE_COLOR)
    private lazy var glt_selectTitleRGBlColor: (r : CGFloat, g : CGFloat, b : CGFloat) = getRGBWithColor(layout.ltTitleLayout.titleSelectColor ?? SELECT_BASE_COLOR)
...
extension LTPageTitleView {
    ...
    private func setupButtonsLayout() {
        ...
        for (_, text) in titles.enumerated() {
            if layout.isAverage {
                let textAverageW = (bounds.width - layout.lrMargin * 2.0 - layout.ltTitleLayout.titleMargin * CGFloat(titles.count - 1)) / CGFloat(titles.count)
                ...
                let textW = text.boundingRect(with: CGSize(width: CGFloat(MAXFLOAT), height: 8), options: .usesLineFragmentOrigin, attributes: [NSAttributedString.Key.font : layout.ltTitleLayout.titleFont ?? UIFont.systemFont(ofSize: 16)], context: nil).size.width
        ...
        let subH: CGFloat = layout.ltTitleLayout.titleHeight == 0 ? bounds.height - layout.bottomLineHeight : layout.ltTitleLayout.titleHeight
        for index in 0..<titles.count {
            let subW = self.layout.ltTitleLayout.titleWidth == 0 ? glt_textWidths[index] : self.layout.ltTitleLayout.titleWidth
            let buttonReact = CGRect(x: upX, y: CGFloat(0.0), width: subW, height: subH)
            let button = subButton(frame: buttonReact, flag: index, title: titles[index], parentView: sliderScrollView)
            button.jk_centerY = self.jk_centerY
            button.layer.masksToBounds = true
            button.layer.cornerRadius = layout.ltTitleLayout.titleCornerRadius ?? 0
            let color = (index == 0 ? layout.ltTitleLayout.titleSelectColor : layout.ltTitleLayout.titleColor)
            let backColor = (index == 0 ? layout.ltTitleLayout.titleSelectBackColor : layout.ltTitleLayout.titleBackColor)
            button.setTitleColor(color, for: .normal)
            button.backgroundColor = backColor
            upX = button.frame.origin.x + subW + layout.ltTitleLayout.titleMargin
        ...
        if layout.sliderWidth == glt_sliderDefaultWidth {
            ...
            if self.layout.ltTitleLayout.titleWidth != 0 {
                var sliderCenter = sliderLineView.center
                let firstBtnCX = firstButton.center.x
                sliderCenter.x = firstBtnCX
                sliderLineView.center = sliderCenter
            }else {
        ...
        let sliderContenSizeW = upX - layout.ltTitleLayout.titleMargin + layout.lrMargin
...
extension LTPageTitleView: LTPageViewDelegate {
    ...
    private func setupButtonStatusAnimation(upButton: UIButton, currentButton: UIButton)  {
        upButton.setTitleColor(layout.ltTitleLayout.titleColor, for: .normal)
        upButton.backgroundColor = layout.ltTitleLayout.titleBackColor
        currentButton.setTitleColor(layout.ltTitleLayout.titleSelectColor, for: .normal)
        currentButton.backgroundColor = layout.ltTitleLayout.titleSelectBackColor
    }
    ...
    private func setupIsClickScrollAnimation(index: Int) {
        ...
        for button in glt_buttons {
            if button.tag == index {
                ...
                button.setTitleColor(self.layout.ltTitleLayout.titleSelectColor, for: .normal)
                button.backgroundColor = self.layout.ltTitleLayout.titleSelectBackColor
            }else {
                if layout.isNeedScale {
                    button.transform = CGAffineTransform(scaleX: 1.0 , y: 1.0)
                }
                button.setTitleColor(self.layout.ltTitleLayout.titleColor, for: .normal)
                button.backgroundColor = self.layout.ltTitleLayout.titleBackColor
            }
        }
...
extension LTPageTitleView {
    ...
    private func subButton(frame: CGRect, flag: Int, title: String?, parentView: UIView) -> UIButton {
        ...
        button.titleLabel?.font = layout.ltTitleLayout.titleFont
